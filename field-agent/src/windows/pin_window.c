/**
 *  pin_window.c      Eleph     May, 2017
 *
 *  Taken from pebble development guides.
 *  Slightly modified to suit our needs.
 *  We use it as a way for the user to input a kragId.
 */
...
static char* selection_handle_get_text(int index, void *context) {
  ...
  snprintf(pin_window->field_buffs[index], sizeof(pin_window->field_buffs[0]), "%x", (int)pin_window->pin.digits[index]);
...
PinWindow* pin_window_create(PinWindowCallbacks callbacks) {
  ...
  if (pin_window) {
    ...
    if (pin_window->window) {
      ...
      // Sub TextLayer... plugged in our string
      ...
      text_layer_set_text(pin_window->sub_text, "Press 'Select' to continue");
      ...
      const GEdgeInsets selection_insets = GEdgeInsets(
        (bounds.size.h - PIN_WINDOW_SIZE.h) / 2.3, 
