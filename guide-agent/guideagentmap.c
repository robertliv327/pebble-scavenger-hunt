...
static void fillbag(void* arg, const char* key, void* item){
  unsigned long p = strtoul(key,NULL,10);
  bag_stats_t* b = arg;
...
static void handle_socket(int comm_sock, struct sockaddr_in *addressp, GA_args_t *args, GA_stats_t * stats,  WINDOW *b, WINDOW *a, int maxx);
...
int main(const int argc, char *argv[])
{
  ...
  while (true) {	      // loop exits on EOF from stdin
	...
	handle_socket(comm_sock, &sAddress, ga_args, ga_stats, b, a, maxx);
...
static void handle_socket(int comm_sock, struct sockaddr_in *sAddress, GA_args_t *args, GA_stats_t * stats, WINDOW *b, WINDOW *a, int maxx)
{
...
// socket has input ready
  ...
  if( (b = newwin(40, maxx, 0,114))==NULL);
...
int quartery)
{
  ...
  //send hint to all              
    ...
    wprintw(c,"Enter one player's name to send the hint to or 'all' to broadcast:\n",response);
...
void parse_args(int argc ,char* argv[], GA_args_t * a){
  ...
  exit(1);
    ...
    char* ptr;
    long ret = strtoul(guideId, &ptr, 10);
...
void find_player_loc(void *arg, const char *key, void *item)
{
  ...
  if (arg!= NULL && item != NULL && key != NULL){
    ...
    float lng= player_get_latitude(p, loc->stats);

    float y= lng-ULNG;
    float num_lines= MMAXY + (y/((ULNG-LLNG)/MMAXY));
    int nl = (int)num_lines;
    
    float x= lat - ULAT;
    float spaces = MMAXX + (x/((ULAT-LLAT)/MMAXX));
    int sp = (int)spaces;
