 ...
 * It’s a set - each key can only occur once in the set - but instead of storing (key,item) pairs, 
 * it tracks a counter for each key. It starts empty. 
 * Each time counters_add is called on a given key, that key’s counter is incremented. 
 ...
 * if key does not exist, create new counter with that key and 
 * initialize the counter to the given value (if this fails, function just returns);
 * Ignore if ctrs is NULL, if key < 0, or count < 0. 
 */
